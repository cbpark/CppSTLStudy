...
int main() {
    ...
    pos = std::find_if(coll.begin(), coll.end(),                 // range
                       std::bind(std::greater<int>(), _1, 3));  // criterion

    // print its position
    std::cout << "the "
              << std::distance(coll.begin(), pos) + 1
              << ". element is the first greater than 3\n";
    ...
    std::cout << "the "
              << std::distance(coll.begin(), pos) + 1
              << ". element is the first divisible by 3\n";

    // find first element not <5
    pos = std::find_if_not(coll.begin(), coll.end(),
                           std::bind(std::less<int>(), _1, 5));
    std::cout << "first value >=5: " << *pos << '\n';
}
