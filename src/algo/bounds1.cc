...
int main() {
    ...
    auto pos1 = std::lower_bound(coll.cbegin(), coll.cend(),
                                 5);
    auto pos2 = std::upper_bound(coll.cbegin(), coll.cend(),
                                 5);
    ...
    coll.insert(std::lower_bound(coll.begin(), coll.end(),
                                 3),
                3);

    // insert 7 at the last possible position without breaking the sorting
    coll.insert(std::upper_bound(coll.begin(), coll.end(),
                                 7),
                7);
